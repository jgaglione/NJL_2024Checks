...
C     
      ...
      DOUBLE PRECISION D1,U1,S1,B1
      DOUBLE PRECISION BX2,SX2,UX2,DX2
...
C     DATA
C     
      DATA D1,U1,S1,B1/4*1D0/
      DATA BX2,SX2,UX2,DX2/4*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      DSIG1=0D0
...
C     Continue only if IMODE is 0, 4 or 5
      ...
      IF (ABS(LPP(IB(1))).GE.1) THEN
        ...
        B1=PDG2PDF(LPP(IB(1)),5, IB(1),XBK(IB(1)),DSQRT(Q2FACT(IB(1))))
      ENDIF
      IF (ABS(LPP(IB(2))).GE.1) THEN
          !LP=SIGN(1,LPP(IB(2)))
        BX2=PDG2PDF(LPP(IB(2)),-5, IB(2),XBK(IB(2)),DSQRT(Q2FACT(IB(2))
     $   ))
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > vm~ c vm c~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > vm~ c vm c~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > vm~ c vm c~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > vm~ c vm c~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
