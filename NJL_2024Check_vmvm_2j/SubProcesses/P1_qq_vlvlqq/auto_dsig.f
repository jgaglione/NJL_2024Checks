...
C     Process: c c~ > vm vm~ c c~ WEIGHTED<=4 @1
C     Process: s s > vm vm~ s s WEIGHTED<=4 @1
C     Process: s s~ > vm vm~ s s~ WEIGHTED<=4 @1
C     Process: c~ c~ > vm vm~ c~ c~ WEIGHTED<=4 @1
C     Process: s~ s~ > vm vm~ s~ s~ WEIGHTED<=4 @1
C     Process: c s > vm vm~ c s WEIGHTED<=4 @1
C     Process: c c~ > vm vm~ s s~ WEIGHTED<=4 @1
C     Process: c s~ > vm vm~ c s~ WEIGHTED<=4 @1
C     Process: s c~ > vm vm~ s c~ WEIGHTED<=4 @1
C     Process: s s~ > vm vm~ c c~ WEIGHTED<=4 @1
C     Process: c~ s~ > vm vm~ c~ s~ WEIGHTED<=4 @1
C     Process: u c > vm vm~ u c WEIGHTED<=4 @1
C     Process: u s > vm vm~ u s WEIGHTED<=4 @1
C     Process: d s > vm vm~ d s WEIGHTED<=4 @1
C     Process: u u~ > vm vm~ c c~ WEIGHTED<=4 @1
C     Process: d d~ > vm vm~ c c~ WEIGHTED<=4 @1
C     Process: b b~ > vm vm~ c c~ WEIGHTED<=4 @1
C     Process: u u~ > vm vm~ s s~ WEIGHTED<=4 @1
C     Process: d d~ > vm vm~ s s~ WEIGHTED<=4 @1
C     Process: b b~ > vm vm~ s s~ WEIGHTED<=4 @1
C     Process: u c~ > vm vm~ u c~ WEIGHTED<=4 @1
C     Process: d c~ > vm vm~ d c~ WEIGHTED<=4 @1
C     Process: b c~ > vm vm~ b c~ WEIGHTED<=4 @1
C     Process: u s~ > vm vm~ u s~ WEIGHTED<=4 @1
C     Process: d s~ > vm vm~ d s~ WEIGHTED<=4 @1
C     Process: b s~ > vm vm~ b s~ WEIGHTED<=4 @1
C     Process: c d > vm vm~ c d WEIGHTED<=4 @1
C     Process: c b > vm vm~ c b WEIGHTED<=4 @1
C     Process: c u~ > vm vm~ c u~ WEIGHTED<=4 @1
C     Process: c d~ > vm vm~ c d~ WEIGHTED<=4 @1
C     Process: c b~ > vm vm~ c b~ WEIGHTED<=4 @1
C     Process: c c~ > vm vm~ u u~ WEIGHTED<=4 @1
C     Process: c c~ > vm vm~ d d~ WEIGHTED<=4 @1
C     Process: c c~ > vm vm~ b b~ WEIGHTED<=4 @1
C     Process: s b > vm vm~ s b WEIGHTED<=4 @1
C     Process: s u~ > vm vm~ s u~ WEIGHTED<=4 @1
C     Process: s d~ > vm vm~ s d~ WEIGHTED<=4 @1
C     Process: s b~ > vm vm~ s b~ WEIGHTED<=4 @1
C     Process: s s~ > vm vm~ u u~ WEIGHTED<=4 @1
C     Process: s s~ > vm vm~ d d~ WEIGHTED<=4 @1
C     Process: s s~ > vm vm~ b b~ WEIGHTED<=4 @1
C     Process: u~ c~ > vm vm~ u~ c~ WEIGHTED<=4 @1
C     Process: u~ s~ > vm vm~ u~ s~ WEIGHTED<=4 @1
C     Process: d~ s~ > vm vm~ d~ s~ WEIGHTED<=4 @1
C     Process: c~ d~ > vm vm~ c~ d~ WEIGHTED<=4 @1
...
C       Output weights and number of events
        ...
        IF (SUMPROB.NE.0D0)THEN
          DO J=1,SYMCONF(0)
            WRITE(*,'(56E12.4)')((SUMWGT(K,I,J)/SUMPROB,K=1,2),I=1
     $       ,MAXSPROC)
          ENDDO
        ENDIF
        SUMPROB=0D0
        ...
        IF (SUMPROB.NE.0D0)THEN
          DO J=1,SYMCONF(0)
            WRITE(*,'(56E12.4)')((NUMEVTS(K,I,J)/SUMPROB,K=1,2),I=1
     $       ,MAXSPROC)
          ENDDO
        ENDIF
        WRITE(*,*)'Events:'
        DO J=1,SYMCONF(0)
          WRITE(*,'(56I12)')((NUMEVTS(K,I,J),K=1,2),I=1,MAXSPROC)
        ENDDO
...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        ...
        ENDIF
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! c c > vm vm~ c c
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! c c~ > vm vm~ c c~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! s s > vm vm~ s s
        IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! s s~ > vm vm~ s s~
        IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! c~ c~ > vm vm~ c~ c~
        IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! s~ s~ > vm vm~ s~ s~
        IF(IPROC.EQ.7) DSIGPROC=DSIG7(P1,WGT,IMODE)  ! c s > vm vm~ c s
        IF(IPROC.EQ.8) DSIGPROC=DSIG8(P1,WGT,IMODE)  ! c c~ > vm vm~ s s~
        IF(IPROC.EQ.9) DSIGPROC=DSIG9(P1,WGT,IMODE)  ! c s~ > vm vm~ c s~
        IF(IPROC.EQ.10) DSIGPROC=DSIG10(P1,WGT,IMODE)  ! s c~ > vm vm~ s c~
        IF(IPROC.EQ.11) DSIGPROC=DSIG11(P1,WGT,IMODE)  ! s s~ > vm vm~ c c~
        IF(IPROC.EQ.12) DSIGPROC=DSIG12(P1,WGT,IMODE)  ! c~ s~ > vm vm~ c~ s~
        IF(IPROC.EQ.13) DSIGPROC=DSIG13(P1,WGT,IMODE)  ! u c > vm vm~ u c
        IF(IPROC.EQ.14) DSIGPROC=DSIG14(P1,WGT,IMODE)  ! u s > vm vm~ u s
        IF(IPROC.EQ.15) DSIGPROC=DSIG15(P1,WGT,IMODE)  ! u u~ > vm vm~ c c~
        IF(IPROC.EQ.16) DSIGPROC=DSIG16(P1,WGT,IMODE)  ! u u~ > vm vm~ s s~
        IF(IPROC.EQ.17) DSIGPROC=DSIG17(P1,WGT,IMODE)  ! u c~ > vm vm~ u c~
        IF(IPROC.EQ.18) DSIGPROC=DSIG18(P1,WGT,IMODE)  ! u s~ > vm vm~ u s~
        IF(IPROC.EQ.19) DSIGPROC=DSIG19(P1,WGT,IMODE)  ! c d > vm vm~ c d
        IF(IPROC.EQ.20) DSIGPROC=DSIG20(P1,WGT,IMODE)  ! c u~ > vm vm~ c u~
        IF(IPROC.EQ.21) DSIGPROC=DSIG21(P1,WGT,IMODE)  ! c c~ > vm vm~ u u~
        IF(IPROC.EQ.22) DSIGPROC=DSIG22(P1,WGT,IMODE)  ! s b > vm vm~ s b
        IF(IPROC.EQ.23) DSIGPROC=DSIG23(P1,WGT,IMODE)  ! s u~ > vm vm~ s u~
        IF(IPROC.EQ.24) DSIGPROC=DSIG24(P1,WGT,IMODE)  ! s s~ > vm vm~ u u~
        IF(IPROC.EQ.25) DSIGPROC=DSIG25(P1,WGT,IMODE)  ! u~ c~ > vm vm~ u~ c~
        IF(IPROC.EQ.26) DSIGPROC=DSIG26(P1,WGT,IMODE)  ! u~ s~ > vm vm~ u~ s~
        IF(IPROC.EQ.27) DSIGPROC=DSIG27(P1,WGT,IMODE)  ! c~ d~ > vm vm~ c~ d~
        IF(IPROC.EQ.28) DSIGPROC=DSIG28(P1,WGT,IMODE)  ! s~ b~ > vm vm~ s~ b~
      ENDIF
